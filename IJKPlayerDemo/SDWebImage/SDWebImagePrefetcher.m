...
- (void)startPrefetchingAtIndex:(NSUInteger)index
{
    ...
    [self.manager downloadWithURL:self.prefetchURLs[index] options:self.options progress:nil completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, BOOL finished)
    {
        ...
        if (image)
        {
//            NSLog(@"Prefetched %li out of %li", self.finishedCount, self.prefetchURLs.count);
        }
        else
        {
//            NSLog(@"Prefetched %li out of %li (Failed)", self.finishedCount, [self.prefetchURLs count]);
...
//    NSLog(@"Finished prefetching (%li successful, %li skipped, timeElasped %.2f)", total - self.skippedCount, self.skippedCount, CFAbsoluteTimeGetCurrent() - self.startedTime);
}
